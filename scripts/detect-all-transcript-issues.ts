 ...
 * 检测1: 对话流程完整性
 * 规则：
 * 1. user后面必须要有assistant
 * 2. toolCall后面必须要有toolResult
 * 3. toolResult后面必须要有assistant
 */
function detectFlowIntegrity(
...
/**
 * 检测2: 已知错误模式（从custom事件和message中提取）
 */
function detectKnownErrors(
  messages: Array<{ lineNum: number; event: MessageEvent }>,
  filePath: string,
  sessionId: string
): Issue[] {
  ...
  for (const msg of messages) {
    ...
    // 检测message中的错误（仅针对assistant角色，排除toolResult）
    if (event.type === 'message' && event.message && event.message.role === 'assistant') {
      // 只检查errorMessage字段，不检查content内容
...
/**
 * 检测3: 异常停止（stopReason不是正常的stop或toolUse）
 */
function detectAbnormalStops(
  messages: Array<{ lineNum: number; event: MessageEvent }>,
  filePath: string,
  sessionId: string
): Issue[] {
...
function analyzeTranscript(filePath: string, accountsMapping: Record<string, EmployeeInfo> | null = null): AnalysisResult {
  ...
  try {
    ...
    // 提取session ID（从文件路径的第一个session事件中提取）
